...
/**
 * PostgreSQL based implementation of {@link StorageService}.
 * 
 * @author Aparna Chaudhary
 */
@Service("postgresStorageService")
public class PostgresStorageService extends JdbcDaoSupport implements StorageService {

	private static final Logger logger = LoggerFactory.getLogger(PostgresStorageService.class);

	@Autowired
	@Qualifier("postgresLobHandler")
	private PostgresLobHandler lobHandler;

	@Autowired
	@Qualifier("healthRecordIncrementer")
	private DataFieldMaxValueIncrementer healthRecordId;

	@Autowired
	@Qualifier("healthRecordDataIncrementer")
	...
	public void save(final DocumentRequest docRequest) {
		...
		logger.info(String.format("Postgres 'store' took %s msec ", lElapsedTime));
	}

	@Override
	public DocumentRequest retrieveContent(String documentType, final String docKey) {
		long lStartTime = System.currentTimeMillis();
		...
		long lElapsedTime = System.currentTimeMillis() - lStartTime;
		logger.info(String.format("Postgres 'getById' for docKey %s took %s msec ", docKey, lElapsedTime));
		return docRequest;
	}
	...
	public ArrayList<DocumentRequest> listDocuments(String documentType, List<FilterCriteria> filterCriteria) {
		...
		long lElapsedTime = System.currentTimeMillis() - lStartTime;
		logger.info(String.format("Postgres 'list' took %s msec ", lElapsedTime));
	...
	class FileMapper implements ParameterizedRowMapper<InputStream> {
		...
		public InputStream mapRow(ResultSet rs, int rowNum) throws SQLException {
			logger.debug("FileMapper:" + rs.getMetaData().getColumnCount());
